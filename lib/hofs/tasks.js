...
function task(pOptions, pSource = {}) {
    ...
    this.run = (pPreviousTaskState = null) => {
        if (!this.options.enabled || this.state.status === STATUS.RUNNING) {
            return Promise.resolve();
        }
            ...
            setTimeout(
                () => {
                    ...
                    this.beforeRun()
                        ...
                        .catch(rErr => {
                            let xError =
                                (rErr && (rErr.detail || rErr.stack || rErr.message || rErr.statusText || rErr)) || '';
                            if (isObject(xError)) {
                            ...
                            if (this.state.retries < this.options.retries && this.state.error !== 'Manual Stop') {
                                ...
                                console.log(
                                    `Retry ${this.state.retries}/${this.options.retries} de ${
                                        this.options.name
                                    } em ${new Date(
                                        new Date().getTime() + this.options.delay * 1000
                                    ).toLocaleString()}s\tErro:${this.state.error}`
                                );
