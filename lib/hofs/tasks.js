...
function task(pOptions, pSource = {}) {
    ...
    this.run = (pPreviousTaskState = null) => {
        console.log(`*-* RUN ${this.options.id}`);
        if (!this.options.enabled || this.isRunning()) {
            if (this.isRunning()) {
                console.log(`*-* RUN ${this.options.id} JÁ EM EXECUÇÃO`);
                this.event.emit('running', true);
            } else {
                console.log(`*-* RUN ${this.options.id} DESABILITADA`);
            }
            return Promise.resolve();
        }
        return new Promise((pResolve, pReject) => {
            ...
            console.log(`*-* RUN ${this.options.id} SETTIMEOUT`);

            setTimeout(
                () => {
                    console.log(`*-* RUN ${this.options.id} SETTIMEOUT RUN`);
                    //Inicia execução
                    //Reseta erro
                    ...
                    this.beforeRun()
                        .then((pIsOk = true) => {
                            console.log(`*-* RUN ${this.options.id} BEFORE`);
                            if (pIsOk) {
                                ...
                                //      EXECUTA
                                //--------------------
                                console.log(`*-* RUN ${this.options.id} EXECUTE`);
                                return this.execute();
                            } else {
                                return null;
                            }
