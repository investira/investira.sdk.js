...
const objects = {
     ...
     */
    objectCompliance: (pObject, pProperties) => {
        if (isEmpty(pObject) || isEmpty(pProperties)) {
            return pObject;
        }
        let xObject = { ...pObject };
        Object.keys(xObject).forEach(xProperty => pProperties.includes(xProperty) || delete xObject[xProperty]);
        return xObject;
    },
     ...
     * @returns Objeto somente com as propriedades válidas
     */
    objectCompliance2: (pModel, pTarget) => {
        ...
        for (const xKey in pTarget) {
            if (pModel.hasOwnProperty(xKey)) {
                //Cria item
                xObject[xKey] = objects.objectCompliance2(pModel[xKey], pTarget[xKey]);
            }
        }
        return xObject;
    },
     ...
     * @returns Objeto somente com as propriedades válidas
     */
    objectCompliance3: (pModel, pTarget) => {
        //Se pTarget não for um objeto, então é um valor ou condição para o Where
        if (!isObject(pTarget) || isNull(pModel)) {
            return pTarget;
        }
        let xObject = {};
    ...
    /**
     * Exclui as propriedades com conteúdo vázio
     *
     * @param {Object} pObject Objeto origem
     * @returns Objeto somente com as propriedades válidas
     */
     ...
     * @param {*} pType Tipo de dado: 'string', 'number', 'date' e 'json'
