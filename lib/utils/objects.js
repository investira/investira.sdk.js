...
const objects = {
     ...
     */
    isEqual(pObj1, pObj2) {
        // Verifica se ambos os valores são estritamente iguais (incluindo null e undefined)
        if (pObj1 === pObj2) return true;

        // Se um dos valores for null ou undefined, e o outro não, eles são diferentes
        if (pObj1 == null || pObj2 == null) return false;

        // Se ambos não forem objetos, compare-os diretamente
        if (typeof pObj1 !== 'object' || typeof pObj2 !== 'object') {
            return pObj1 === pObj2;
        }

        // Obtém as chaves de ambos os objetos
        const xKeys1 = Object.keys(pObj1);
        const xKeys2 = Object.keys(pObj2);

        // Cria um conjunto de todas as chaves para garantir que todas sejam verificadas
        const allKeys = new Set([...xKeys1, ...xKeys2]);

        // Percorre cada chave no conjunto
        for (let key of allKeys) {
            const xHasKey1 = pObj1.hasOwnProperty(key);
            const xHasKey2 = pObj2.hasOwnProperty(key);

            // Se ambos os objetos possuem a chave, compare os valores recursivamente
            if (xHasKey1 && xHasKey2) {
                if (objects.isEqual(pObj1[key], pObj2[key])) return false;
            } else {
                // Se uma das chaves estiver ausente em um dos objetos, eles são diferentes
                return false;
            }
        }

        // Se nenhuma diferença foi encontrada, os objetos são iguais
        return true;
    }
};
