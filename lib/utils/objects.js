    ...
    defaultArrayMerge: (pTarget, pSource, pOptionsArgument) => {
        ...
        pSource.forEach((e, i) => {
            if (isUndefined(xDestination[i])) {
                ...
                xDestination[i] = objects.deepMerge(
                    pTarget[i],
                    e,
                    pOptionsArgument
                );
                ...
                xDestination.push(
                    objects.cloneIfNecessary(e, pOptionsArgument)
                );
            }
        });
        return xDestination;
    ...
    mergeObject: (pTarget, pSource, pOptionsArgument) => {
        ...
        if (isMergeable(pTarget)) {
            Object.keys(pTarget).forEach(xKey => {
                xDestination[xKey] = objects.cloneIfNecessary(
                    pTarget[xKey],
                    pOptionsArgument
                );
            });
        ...
        Object.keys(pSource).forEach(xKey => {
            if (!isMergeable(pSource[xKey]) || !pTarget[xKey]) {
                ...
                xDestination[xKey] = objects.deepMerge(
                    pTarget[xKey],
                    pSource[xKey],
                    pOptionsArgument
                );
