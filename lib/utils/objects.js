...
const objects = {
    ...
    /**
     * Retorna as propriedades preenchidas do objeto novo que estejam
     * ausentes ou vazias no objeto atual.
     *
     * A comparação é realizada recursivamente para propriedades que
     * sejam objetos simples.
     *
     * Valores já preenchidos no objeto atual não são substituídos,
     * mesmo que sejam diferentes dos valores existentes no objeto novo.
     *
     * @param {Object} pObjetoAtual Objeto atual utilizado como referência.
     * @param {Object} pObjetoNovo Objeto contendo os novos valores candidatos.
     * @returns {Object} Objeto contendo apenas as propriedades faltantes.
     */
    getMissingProperties: (pObjetoAtual, pObjetoNovo) => {
        /**
         ...
         *
         * Valores booleanos false e numéricos 0 não são considerados vazios.
         *
         ...
         *
         * Arrays e null não são considerados objetos simples
         * para efeito da comparação recursiva.
         *
         * @param {*} pValor Valor a ser analisado.
         * @returns {boolean} true quando o valor for um objeto simples.
         */
        function pvEhObjeto(pValor) {
            return pValor !== null && typeof pValor === 'object' && !Array.isArray(pValor);
        }

        /**
         * Normaliza datas para comparação independente do horário UTC.
         *
         * @param {*} pValor Valor a ser normalizado.
         * @returns {*} Valor original ou a data no formato YYYY-MM-DD.
         */
         ...
         *
         * Percorre somente as propriedades existentes no objeto novo.
         * Quando uma propriedade do objeto novo também for um objeto,
         * seus itens filhos são comparados recursivamente.
         *
         * @param {Object} pAtual Objeto atual utilizado como referência.
         * @param {Object} pNovo Objeto contendo os novos valores.
         * @returns {Object} Propriedades preenchidas no novo e faltantes no atual.
         */
        function pvComparar(pAtual, pNovo) {
            ...
            Object.keys(pNovo).forEach(pChave => {
                const xValorNovo = pvNormalizarData(pNovo[pChave]);
                const xValorAtual = pvNormalizarData(pvEhObjeto(pAtual) ? pAtual[pChave] : undefined);

                /*
                 * Objetos são tratados antes da verificação de vazio
                 * para permitir a comparação recursiva dos itens filhos.
                 */
                if (pvEhObjeto(xValorNovo)) {
                    const xObjetoAtual = pvEhObjeto(xValorAtual) ? xValorAtual : {};

                    const xDiferenca = pvComparar(xObjetoAtual, xValorNovo);

                    /*
                     * Somente inclui o objeto pai quando alguma diferença
                     * válida tiver sido encontrada em seus itens filhos.
                     */
                    if (!pvEstaVazio(xDiferenca)) {
                ...
                /*
                 * Nunca utiliza valores vazios provenientes
                 * do objeto novo.
                 */
                ...
                /*
                 * Adiciona o valor novo somente quando a propriedade
                 * correspondente no objeto atual estiver vazia
                 * ou não existir.
                 */
                if (pvEstaVazio(xValorAtual) && xValorAtual !== xValorNovo) {
                    xResultado[pChave] = xValorNovo;
                }
            });
