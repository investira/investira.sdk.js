...
const { BadRequestError } = require('../messages/ClientErrors');
...
const httpRequests = {
    ...
    /**
     * Valida campos obrigatórios e lança BadRequestError quando faltarem.
     * @param {object} pData Objeto com dados de entrada.
     * @param {string[]} [pFields=[]] Lista de campos obrigatórios.
     */
    assertFields: (pData, pFields = []) => {
        // Interrompe cedo quando faltar qualquer campo obrigatório.
        for (const xField of pFields) {
            if (isEmpty(pData?.[xField])) {
                throw new BadRequestError(`É necessário informar ${xField}`);
            }
        }
    }
};
