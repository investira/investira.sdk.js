...
const httpRequests = {
    ...
    request: (pMethod, pProps, pFullResponse = false) => {
        return new Promise((pResolve, pReject) => {
            ...
            if (pProps.hasOwnProperty('pProps.onUploadProgress')) {
            ...
            baseRequest(xConfig)
                ...
                .catch(rErr => {
                    if (rErr.response && rErr.response.data.error && rErr.response.data.error.code.status >= 500) {
                        pReject(rErr);
                    } else if (!rErr.response) {
                        pReject(new NoResponse(rErr.message));
                    } else {
                        if (pFullResponse) {
                            pReject(rErr.response);
                        } else {
                            pReject({
                                ...rErr.response.data,
     ...
     * @return {Promise}
     */
    hasConection: async (pUrl = 'https://investira.com.br') => {
        const xProps = { url: pUrl };
     ...
     */
    cancelToken: () => {
        return baseRequest.CancelToken.source();
    }
};
