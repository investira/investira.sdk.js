...
const dates = {
    ...
    isHoliday: pDate => {
        ...
        const xDateMD = formatDateCustom(pDate, '-MM-DD');
        const xDateMovel = pDate.getFullYear() + xDateMD;
        const xDateFixo = 'XXXX' + xDateMD;
        let xHoliday =
            xLocale._holidays &&
            (xLocale._holidays.indexOf(xDateMovel) >= 0 ||
                xLocale._holidays.indexOf(xDateFixo) >= 0);
        //Ignora se feriado cair em dia não útil
        if (xHoliday && xLocale._notBusinessDays.indexOf(pDate.getDay()) >= 0) {
            xHoliday = false;
        }
        return xHoliday;
    },
