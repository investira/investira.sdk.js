...
const dates = {
     ...
     * Se feriado cair em um dia normalmente não util(fim de semana), retornará false.
     *
     * @param {Date} pData
     * @returns {boolean}
     */
    isHoliday: pDate => {
        ...
        const xDateYMD = formatDateCustom(pDate, HOLIDAY_FORMAT);
    ...
    holidays: (pDate1, pDate2) => {
        ...
        const xI1 = seekDate(
            xLocale,
            formatDateCustom(dates.addDays(pDate1, 1), HOLIDAY_FORMAT),
            true
        );
        //Procura feriado anterior mais próximo a data final
        const xI2 = seekDate(
            xLocale,
            formatDateCustom(pDate2, HOLIDAY_FORMAT),
            false
        );
     ...
     * @param {Date} pDate1
     * @param {Date} pDate2
     * @param {boolean} [pWorkingDays=false] Somente dias úteis
     * @returns {number}
     */
    daysBetweenProrata: (pDate1, pDate2, pWorkingDays = true) => {
        const xD = dates.diff('day', pDate1, pDate2, true);
        if (pWorkingDays) {
            const xW = dates.weekends(pDate1, pDate2);
            const xH = dates.holidays(pDate1, pDate2);
            return xD - xW - xH;
        } else {
            return xD;
        }
...
const pvLoadDataPush = (pHolidays, pKey, pHolidayString) => {
    ...
    //Exclui da lista feriado que cair em dia não considerado útil
    if (
        dataHolidays[pKey].notBusinessDays.indexOf(xHolidayDate.getUTCDay()) ==
        -1
    ) {
