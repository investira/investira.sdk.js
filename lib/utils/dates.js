...
const dates = {
    ...
    dateToObject: pDate => {
        ...
        const xNowString = pDate.toISOString();
        return {
            year: xNowString.substr(0, 4),
            month: xNowString.substr(5, 2),
            day: xNowString.substr(8, 2),
            hour: xNowString.substr(11, 2),
            minute: xNowString.substr(14, 2),
            second: xNowString.substr(17, 2)
        };
    },
    // { "type": "D", "time": "08:00" },
    ...
    scheduleToDate: pSchedule => {
        ...
        const xCurrentDate = dates.toDate();
        ...
        if (pSchedule.type === 'D') {
            ...
            const xDif = pSchedule.weekday - xCurrentDate.getDay();
            let xNextDay = numbers.toNumber(xNow.day);
            //Adiciona os dias até ser o dia da semana desejado
            if (xDif < 0) {
                xNextDay += 7 - Math.abs(xDif);
            } else if (xDif > 0) {
                xNextDay += xDif;
            }
            ...
            if (xNextDate < xCurrentDate) {
                ...
                xNextDay = ('0' + (numbers.toNumber(xNextDay) + 7)).slice(-2);
                xNextDate = dates.toDate(`${xNow.year}${xNow.month}${xNextDay}${pSchedule.time}`, SCHEDULE_FORMAT);
            }
