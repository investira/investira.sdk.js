...
const { isEmpty, isUndefined } = require('./validators');
...
const dates = {
    ...
    toDate: (pDate, pFormat = null) => {
        if (!isUndefined(pDate) && !isUndefined(pFormat)) {
            if (isEmpty(pDate) || isEmpty(pFormat)) {
                return null;
            } else {
    ...
    holidays: (pDate1, pDate2) => {
        ...
        const xI1 = seekDate(xLocale, pvToHolidayFormat(dates.addDays(pDate1, 1)), true);
