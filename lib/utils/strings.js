...
const strings = {
    ...
    toTitleCase: pString => {
        ...
        const xToFullUpperCase = ['i', 'ii', 'iii', 'iv', 'v'];
        ...
        let xResult = '';
        for (let xI = 0; xI < xLista.length; xI++) {
            let xPalavra = xLista[xI];
            if (xIgnore.includes(xPalavra)) {
                xResult += xPalavra + ' ';
            } else if (xToFullUpperCase.includes(xPalavra)) {
                xResult += xPalavra.toUpperCase() + ' ';
            } else {
                xResult +=
                    xPalavra.charAt(0).toUpperCase() +
                    xPalavra.slice(1).toLowerCase() +
                    ' ';
            }
        }
        return xResult.trim();
