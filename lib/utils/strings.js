...
const strings = {
    ...
    /**
     * Retorna a array com palavras ou frases para serem utlizados em query.
     * Frase devem ser separada por vígula.
     * Ignora caracter individual.
     *
     * @param {string} pString
     * @returns {Array}
     */
    stringToQuery: pString => {
        ...
        for (const xItem of xItens) {
            //Salva quando for somente um caracter
            if (xItem.length > 1) {
                xPesquisa.push(`%${xItem}%`);
            }
        }
        return xPesquisa;
    }
