...
const formats = {
     ...
     *
     * @param {number} pValue Quantidade de meses
     * @returns
     */
    friendlyDate: pValue => {
        ...
        const xYears = Math.floor(pValue / 12);
        const xMonths = pValue % 12;
    ...
    friendlyBetweenDates: (pDate1, pDate2) => {
        let xPeriod = '';
        if (isEmpty(pDate1) || isEmpty(pDate2)) return xPeriod;

        let xDate1 = pDate1;
        let xDate2 = pDate2;
        const daysMonth = daysInMonth(xDate1);

        if (isString(pDate1)) xDate1 = toDate(xDate1);
        if (isString(pDate2)) xDate2 = toDate(xDate2);

        if (daysBetween(xDate1, xDate2) <= 0) {
            return '';
        }

        const xYearsAbs = yearsBetween(xDate1, xDate2);
        const xMonthsAbs = monthsBetween(xDate1, xDate2) % 12;
        const xDaysAbs = (xMonthsAbs * daysMonth) % daysMonth;
        const xYears = Math.floor(xYearsAbs);
        const xMonths = Math.floor(xMonthsAbs);
        const xDays = Math.ceil(xDaysAbs);
