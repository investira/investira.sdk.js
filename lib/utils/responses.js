...
const responses = {
     ...
     * @param {*} pClauses
     * @param {*} pTotalItensPromise Promise que retorna a quantidade total de itens
     * @param {object} pArrayToObjectParams Parametros para a criação da chave dos objetos
     * - keyPropertyName
     * - keyFormatFunction
     * - keyPrefix
     * @returns {Promise}
     */
    serviceDataResponse: (pMainResult, pClauses, pTotalItensPromise, pArrayToObjectParams) => {
        ...
        if (pClauses && pClauses.limit && pClauses.limit.size) {
            ...
            if (pTotalItensPromise && typeof pTotalItensPromise === 'function') {
