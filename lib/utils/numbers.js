const { isNumber, isString, isEmpty } = require('./validators');
...
const numbers = {
     ...
     */
    sumValues: pValues => {
        return pValues.reduce((p, c) => numbers.add(p, c), 0);
    },
    ...
    weightedMean: (pValues, pWeights) => {
        let xArray = pValues.map((pValue, index) => numbers.mul(pValue, pWeights[index]));
        return numbers.div(numbers.sumValues(xArray), numbers.sumValues(pWeights));
    },
    ...
    apart: pValue => {
        ...
        if (xSN > 0) {
            xValueStr = Math.abs(pValue).toFixed(Number(xValueStr.substr(xSN + 2, 10)));
