...
const numbers = {
    ...
    percentile: (pValues, pPercentil, pInclusive = false) => {
        ...
        if (pInclusive) {
            ...
            // Calcular a posição usando o método exclusivo
        ...
        if (pInclusive) {
            ...
            if (xBase < 0) {
                // Percentil abaixo do menor valor
                return xSorted[0];
            } else if (xBase >= n - 1) {
                // Percentil acima do maior valor
                return xSorted[n - 1];
            } else {
                // Interpolação linear
                return xSorted[xBase] + xRest * (xSorted[xBase + 1] - xSorted[xBase]);
