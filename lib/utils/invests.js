...
const invests = {
     ...
     */
    aposentadoria: (pData, pTarget, pAbort = () => false) => {
        return new Promise((pResolve, pReject) => {
            setTimeout(() => {
                pvAposentadoriaAtingirMeta(pData, pTarget, pAbort)
                    .then(rResult => {
                        pResolve(rResult);
                    })
                    .catch(rErr => {
...
async function pvAposentadoriaAtingirMeta(pData, pTarget, pAbort) {
    try {
        let xMenorValor =
        ...
        if (pTarget == pData.sac.pv) {
            pTarget.value = 0;
            xCalculo = pvCalculaEcoSac(pData);
...
function pvCalculaEcoSac(pData) {
    let xFV = 0;
    //Lista crédito
